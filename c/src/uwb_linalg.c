...
int uwb_sym_eig(uwb_real *a, uwb_real *eig, uwb_real *vec, int n)
{
     ...
     * vec が NULL なら固有ベクトルは計算しない (Beck 法は最大固有値しか
     * 使わないので、その呼び方が既定)。 */
    int sweep, i, j, k;
    if (n <= 0 || n > UWB_LA_MAX) return 0;
    if (n == 1) { eig[0] = a[0]; if (vec) vec[0] = (uwb_real)1; return 1; }

    if (vec) {
        for (i = 0; i < n; ++i)
            for (j = 0; j < n; ++j) vec[IDX(i, j, n)] = (i == j) ? (uwb_real)1 : (uwb_real)0;
    }
    ...
    for (sweep = 0; sweep < 60; ++sweep) {
        ...
        for (i = 0; i < n; ++i) {
            for (j = i + 1; j < n; ++j) {
                ...
                if (vec) {
                    for (k = 0; k < n; ++k) {
                        uwb_real vki = vec[IDX(k, i, n)], vkj = vec[IDX(k, j, n)];
                        vec[IDX(k, i, n)] = c * vki - s * vkj;
                        vec[IDX(k, j, n)] = s * vki + c * vkj;
                    }
                }
                for (k = 0; k < n; ++k) {
    ...
    /* 昇順に並べる (挿入ソート。n <= 9 なので十分)。vec があれば固有値と
     * 同じ動かし方で列も一緒にずらす (最後にまとめて並べ替えたりしない)。 */
    for (i = 1; i < n; ++i) {
        uwb_real v = eig[i];
        uwb_real vcol[UWB_LA_MAX];
        int kk;
        if (vec) for (kk = 0; kk < n; ++kk) vcol[kk] = vec[IDX(kk, i, n)];
        for (j = i - 1; j >= 0 && eig[j] > v; --j) {
            eig[j + 1] = eig[j];
            if (vec) for (kk = 0; kk < n; ++kk) vec[IDX(kk, j + 1, n)] = vec[IDX(kk, j, n)];
        }
        eig[j + 1] = v;
        if (vec) for (kk = 0; kk < n; ++kk) vec[IDX(kk, j + 1, n)] = vcol[kk];
    }
    return 1;
}
