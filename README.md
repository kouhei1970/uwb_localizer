...
|---|---|---|---|---|---|
| 3 台 | **解けない** | — | — | — | 0 % |
| 4 台 | 0.961 m | 1.114 m | 0.564 m | **2.428 m** | 88.0 % |
| 5 台 | 0.859 m | 0.944 m | 0.542 m | 0.756 m | 99.2 % |
| **6 台** | 0.464 m | 0.694 m | **0.232 m** | 0.467 m | 100 % |
| 8 台 | 0.275 m | 0.366 m | 0.159 m | 0.233 m | 100 % |

読み取れること:

- **3D なら最低 4 台**。3 点は必ず同一平面上に乗るので、3 台では原理的に
  高さが決まらない (この場合ライブラリは解を返さない)
- **4 台は「解ける」だけで実用ではない。** 1 本欠けるとすぐ解けなくなるので
  測位率が 88% まで落ちる
- **4 台では Lv3 が「速いが時々外す」。** 中央値では Lv3 が常に勝つが、4 台では
  最悪ケースが 2.4 m まで伸びた。幾何が痩せたところに欠測が重なると、EKF が
  一度ずれてから戻るのに時間がかかる。**外れないことが大事なら 4 台では Lv2**
- **6 台から悩まなくてよくなる。** 測位率 100%、Lv3 が中央値も最悪も上回る

数字は `python examples/05_choose_level.py` でそのまま再現できる
(この表と同じ配置・同じ 8 seed)。
...
**距離は ANCHOR 側の UART にしか出ない。** ここが配置を決める。
...
**TAG に繋いだホストは距離を受け取れない。** 送信元アドレスすら入らないので、
TAG は「誰かに読まれた」ことしか分からない。一般的な UWB システム
(タグは小さく安く、アンカーは有線給電) の感覚とは逆になる。

> 仕様書 16 / 17 節から読み取れることで、**実機では確認していない**。
...
**移動体の上で自己位置を知る用途には使えない** (TAG 側に距離が出ないため)。
集約した結果を無線などで移動体に送り返すことになる。
...
**ただし台数が少ないと Lv3 は最悪ケースが伸びる** (上の「何台買えばいいか」の表)。
中央値では Lv3 が勝つが、4〜5 台では時々大きく外す。外れないことが大事なら
Lv2 の方が読める。
...
**全部ハードなしで動く。** 解説は [examples/README.md](examples/README.md)。

```bash
python examples/01_quickstart.py            # Lv0-Lv3 を同じ観測で比較
python examples/02_custom_hal.py            # 自前の UWB 用 HAL を書く
python examples/03_minimal_integration.py   # つなぎ方 3 通りの最小形
python examples/04_anchor_layout.py         # 置き方と台数で精度がどう変わるか
python examples/05_choose_level.py          # 測位レベルの選び方
python examples/06_troubleshooting.py       # 壊れ方から原因を当てる
```
...
```bash
pip install -e ".[dev]"
python -m pytest -q      # 176 件 (C 版の突き合わせを含む)
```
