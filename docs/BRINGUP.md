...
> **この文書**: 実機で何を用意し、何を渡せばよいか (モジュール別の目安つき)。RYUW122 固有の手順は [RYUW122.md](RYUW122.md)、データ形式の仕様は [UWB_PROTOCOL.md](UWB_PROTOCOL.md)。 → [ドキュメント一覧](README.md)
...
```python
# (a) 巻き尺で測る
anchors = [ul.Anchor("A0", [0.2, 0.2, 2.4])]        # 台数分ならべる

# (b) 相互測距から推定して、実測した 4 台以上に合わせる
...
|---|---|---|
| **REYAX RYUW122 / _Lite** | ANCHOR の UART に距離が cm で出る<br>(`+ANCHOR_RCV`) | **専用 HAL あり。ポートを指定するだけ**<br>→ [RYUW122.md](RYUW122.md) |
...
ハードが無くても [ブラウザ UI](TUTORIAL.md#第-1-章-とりあえず動かす) で
