...
天井の 4 隅に貼っただけだと 2 つ問題が起きる。

1. 高さがほとんど観測できず、その誤差が水平方向にも回り込む
2. **鏡像解**が生じる。平面の上下に、全アンカーからの距離が厳密に一致する
   2 点ができるので、測距値だけではどちらか選べない。放っておくと
   水平は正しいまま高さだけ丸ごと折り返る

この構成で 3D の測位器を作ると警告が出る。対処は 2 つ。

```python
ul.SolveConfig(dim=2, z_fixed=1.2)          # 高さを固定する (確実)
ul.SolveConfig(dim=3, z_bounds=(0.0, 2.3))  # 「天井より下」を事前情報として与える
```

どちらも取れない場合、測位結果の `fix.ambiguous` が True なら
「高さが折り返っているかもしれない」という意味になる。
...
- 「ライブ」タブから実機の観測を流し込んで表示。推定 σ の円が位置と一緒に描かれる

### ライブで位置が出るのに要るもの

**2 つだけ。**

1. **観測源** — ファイル / TCP / シリアルのどれか。形式は
   **テキスト (正規表現)** か **JSON Lines**。テキストなら `sniff` が出した
   正規表現・単位・ID 接頭辞をそのまま貼る
2. **アンカー座標** — 左パネルで置いたものが使われる。JSON Lines の場合は
   観測源から `anchors` メッセージで送ってもよい (その場合は左パネル不要)

レベルは既定 Lv2。Lv3 を選ぶなら時刻が要るので、ファームが時刻を出さない
ログを再生するときは「時刻が無いので一定レートとみなす」にチェックする。

位置が出ないときは、状態表示に出る**解釈行数 / 捨てた行数**を見る。

| 表示 | 意味 |
|---|---|
| 解釈 0 行 / 捨てた N 行 | 行は読めているが正規表現が当たっていない |
| 解釈 0 行 / 捨てた 0 行 | そもそも読めていない (ポート・パスを確認) |
| 「アンカー座標がありません」 | 左パネルにアンカーが無い |
| 解釈できているのに位置が出ない | アンカー ID の不一致、または単位の取り違え |
...
| 場所 | 何が出ているか |
|---|---|
| **平面図 (X-Y)** | 部屋を真上から見た図。灰色が真値の軌跡、色付きが各レベルの推定。青い四角がアンカー。`GDOP` にチェックを入れると幾何精度の悪い場所が赤く塗られる |
| **側面図 (X-Z)** | 横から見た図。**高さが解けているかはここを見る**。アンカーが同一平面だと推定が上下に暴れる |
| **精度** | 測位率・RMSE・CEP50/95・最大誤差。下に CRLB (理論下限) と NLOS 率 |
| **誤差の時系列** | いつ大きく外れたか。NLOS の遮蔽が始まった瞬間が山になって見える |
| **誤差の累積分布** | 横軸の誤差以下に収まる割合。破線が 50% と 95%。**裾の長さ**がここでいちばん分かりやすい |

再生ボタンとスライダーで時刻をたどれる。丸が各レベルの推定位置、黒い輪が真値。

### 触り方

1. 左の「部屋とアンカー」でプリセットを選ぶか、平面図を直接クリック／タップして置く
2. 誤差モデルのスライダーを振る (まず `NLOS 確率` を 0 → 0.3 にすると差が出る)
3. 「レベル」で比べたいものにチェック
4. **実行**

アンカーは平面図上でドラッグして動かせる (指でも可)。高さは左の一覧の 3 つ目の欄で編集する。
削除は Shift+クリック、または一覧の `×`。

### スマホ・タブレットから使う

1 列レイアウトに切り替わり、指でのドラッグとタップにも対応している。
ただし**既定では localhost にしか公開していない**ので、同じ LAN の端末から開くには
明示的にアドレスを開ける。

```bash
python -m uwb_loc ui --host 0.0.0.0
```

```
UWB 測位 UI: http://0.0.0.0:8765
  同じ LAN の端末からは http://192.168.1.23:8765
  警告: LAN 全体に公開されます (認証なし)。信用できる回線でのみ使ってください。
```

表示された LAN アドレスをスマホのブラウザに入れる。**認証は一切ないので、
同じネットワークにいる誰でも開けてしまう**。現場の共有 Wi-Fi や公衆回線では使わないこと。

スマホでの制約が 2 つある。

- 削除の Shift+クリックが使えない → アンカー一覧の `×` を使う
- 設定パネルが縦に長い。「実行」を押すと自動で平面図まで送るようにしてあるが、
  設定を変えるたび上下に動くことにはなる

腰を据えて配置を検討するならデスクトップ、現場でアンカーを動かしながら
確認するならスマホ、という使い分けが現実的。
